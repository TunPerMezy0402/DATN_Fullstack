...
const Register: React.FC = () => {
  ...
  const {
    ...
    onSuccess: (response) => {
      ...
      setErrors((prev) => ({
        ...prev,
        general: 'Đăng ký với Google thành công! Đang chuyển hướng...',
      }));
    ...
    onError: (error) => {
      setErrors((prev) => ({
        ...prev,
        general: error.message || 'Đăng ký với Google thất bại! Vui lòng thử lại.',
      }));
    },
    isRegister: true,
  });
  ...
  const handleGoogleRegister = async () => {
    ...
    if (!isGoogleLoaded) {
      // Try to initialize Google if not loaded
      const initialized = await initializeGoogle();
      if (!initialized) {
        setErrors((prev) => ({
          ...prev,
          general: 'Google OAuth chưa được tải. Vui lòng thử lại.',
        }));
        return;
      }
    }
    ...
    try {
      ...
      console.error('Google register trigger error:', error);
    } finally {
      setLoading(false);
    }
  };
